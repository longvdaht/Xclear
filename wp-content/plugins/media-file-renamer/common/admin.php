...
if ( !class_exists( 'MeowKit_MFRH_Admin' ) ) {

  class MeowKit_MFRH_Admin {
    ...
    public function __construct( $prefix, $mainfile, $domain, $isPro = false, $disableReview = false, $freeOnly = false ) {
      ...
      if ( is_admin() ) {
        ...
        if ( MeowKit_MFRH_Helpers::is_asynchronous_request() ) {
          return;
        }
    ...
    public static function process_deferred_instances() {
      ...
      if ( !function_exists( 'current_user_can' ) || !function_exists( 'wp_get_current_user' ) ) {
        trigger_error(
          'MeowKit_MFRH_Admin: Pluggable functions still unavailable on init hook. ' .
    ...
    private function run_admin_setup() {
      ...
      if ( !MeowKit_MFRH_Admin::$loaded ) {
        // Check for potential issues with WordPress install, other plugins, etc.
        new MeowKit_MFRH_Issues( $this->prefix, $this->mainfile, $this->domain );
        ...
        MeowKit_MFRH_Admin::$loaded = true;
      }
      ...
      if ( $this->is_user_admin() ) {
        if ( !$this->disableReview ) {
          new MeowKit_MFRH_Ratings( $this->prefix, $this->mainfile, $this->domain );
        }
        new MeowKit_MFRH_News( $this->domain );
      }
    }
    ...
    public function admin_menu_start() {
      ...
      if ( !isset( $submenu[ 'meowapps-main-menu' ] ) ) {
        add_menu_page(
          'Meow Apps',
          '<img alt="Meow Apps" class="meowapps-menu-icon" src="' . MeowKit_MFRH_Admin::$logo . '" />Meow Apps',
          'manage_options',
          'meowapps-main-menu',
    ...
    public function meowapps_hide_dashboard_callback() {
      ...
      echo MeowKit_MFRH_Helpers::wp_kses( $html );
    }
