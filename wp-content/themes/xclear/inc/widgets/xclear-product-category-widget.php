...
class XclearProductCategoryWidget extends \Elementor\Widget_Base
{
    public function get_name(): string
    {
        return 'xclear_product_category_widget';
    }

    public function get_title(): string
    {
        return esc_html__('Xclear Product Category', 'xclear');
    }
    ...
    protected function render(): void
    {
        ...
        // Read sort from category ACF field, fallback to 'default' (position-based)
        $sort        = 'default';
        $customOrder = [];
        if ($term instanceof \WP_Term) {
            $sort = get_field('product_category_default_sort', $term) ?: 'default';
            if ($sort === 'custom') {
                $customOrder = (array) (get_field('product_category_custom_order', $term) ?: []);
            }
        }

        $query_args  = xclear_build_products_query($term_slug, $sort, 1, $per_page, $customOrder);
